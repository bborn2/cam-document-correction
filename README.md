...
| `onnx` | **YOLOv8-OBB 模型**，旋转框输出 | onnxruntime |
...
> 本项目的 `best-30.onnx` 是在自标注 A4 数据集上训练的单类 OBB 模型（类别 `document`，显示为 `A4`）。
...
# 用 OBB 模型
uv run a4detect --detector onnx --weights best-30.onnx

# 不用模型，传统 CV
uv run a4detect

# 指定摄像头 / 调阈值
uv run a4detect --detector seg --weights best.onnx --camera 1 --conf 0.3
```
...
- ONNX：`--conf`（置信度阈值，默认 0.25）、`--iou`（NMS，默认 0.45）。
